...
public class HttpProxyServer {
	...
	public void listen() {
		new Thread(() -> {
			while (running.get() && (serverSocket != null) && serverSocket.isBound()) {
				try {
					serviceThreads.add(new RequestHandler(serverSocket.accept()));
				} catch (SocketTimeoutException e1) {
					/* Nothing to do here */ } catch (IOException e2) {
					e2.printStackTrace();
				}
			}
		}).start();
	...
	private void loadBlockSites() {
		...
		if (!blockedSitesSourceFile.exists()) {
			...
			try {
				blockedSitesSourceFile.createNewFile();
			} catch (Exception e1) {
				/* Nothing to do here */ }
		} else {
		...
		if (!blockedSitesListFile.exists()) {
			...
			// Create file and inform user
			try {
				blockedSitesListFile.createNewFile();
			} catch (Exception e1) {
				/* Nothing to do here */ }
		} else {
			// Create a fileStream for file to list converting
		...
		for (String tempSource : this.blockedSource) {
			...
			try {
				...
				while ((tempLine = inputReader.readLine()) != null) {
					...
					if ((tempLine.length() < 3) || (tempLine.startsWith("#")) || (tempLine.contains("localhost"))
	...
	private final class ConsoleInterface extends Thread {
		public void run() {
			...
			while (running.get()) {
				System.out.println(
						"Enter new site to block, or type \"blocked\" to see blocked sites, \"connections\" to see current connection count, or \"close\" to close server.");
	...
	private final class RequestHandler extends Thread {
		...
		public void run() {
			try {
				...
				if (this.isHttpsTunnel(request[0]))
					this.connectTunnel(request[1]);
				else
					this.connectRelay(req);
		...
		private void connectRelay(String remoteAddress) {
			try {
				...
				// String contentType = proxyToServer.getHeaderField("Content-Type");

				// Inform client of success
				// this.relayRequested();

				// cked by CORS policy: No 'Access-Control-Allow-Origin' header is present on
				// the requested resource.

				// "Access-Control-Allow-Origin: *"

				try {
					this.proxyToClientOutput.write("HTTP/1.0 200 OK\n" + "Proxy-agent: ProxyServer/1.0\n"
							+ "Access-Control-Allow-Origin: *\n" + "\r\n");
		...
		private void blockedSiteRequested() {
			try {
				this.proxyToClientOutput
						.write("HTTP/1.0 403 Access Forbidden \n" + "User-Agent: ProxyServer/1.0\n" + "\r\n");
		...
		private void notFoundSiteRequest() {
			try {
				this.proxyToClientOutput.write("HTTP/1.0 404 NOT FOUND\n" + "Proxy-agent: ProxyServer/1.0\n" + "\r\n");
		...
		private void closeConnection(Socket toClose1, Socket toClose2) {
			...
			// Close Streams
			try {
				if (this.proxyToClientInput != null)
					this.proxyToClientInput.close();
			} catch (Exception e) {
				/* Nothing to do here */ }

			try {
				if (this.proxyToClientOutput != null)
					this.proxyToClientOutput.close();
			} catch (Exception e) {
				/* Nothing to do here */ }
